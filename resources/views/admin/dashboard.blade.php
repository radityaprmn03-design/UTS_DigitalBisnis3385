@extends('layouts.admin')
     ...
     @section('content')
     <!-- Quick Action Banner -->
     <div class="mb-8 p-6 bg-gradient-to-r from-slate-900 via-indigo-950 to-slate-900 rounded-3xl text-white flex flex-col md:flex-row justify-between items-center shadow-xl border border-slate-800">
         <div class="mb-4 md:mb-0">
             <span class="px-3 py-1 bg-indigo-500/20 text-indigo-300 rounded-full text-xs font-bold uppercase tracking-wider">Gate Check-in Hari-H</span>
             <h2 class="text-2xl font-black mt-2">🛡️ Scanner QR Code Penjaga Pintu</h2>
             <p class="text-slate-400 text-sm mt-1">Scan E-Ticket peserta di pintu masuk via kamera HP/Webcam untuk mencegah double entry.</p>
         </div>
         <a href="{{ route('admin.checkin.index') }}" class="px-6 py-4 bg-indigo-600 hover:bg-indigo-500 text-white font-extrabold rounded-2xl transition shadow-lg shadow-indigo-900 whitespace-nowrap">
             Buka Scanner Registrasi →
         </a>
     </div>
     ...
     <!-- Chart Section -->
     <div class="bg-white rounded-3xl border border-slate-100 shadow-sm mb-10 overflow-hidden p-8">
         <h3 class="font-black text-xl mb-4">Grafik Pertumbuhan Penyelenggaraan Event Tahun Ini</h3>
     ...
     <script>
        ...
        const eventGrowthChart = new Chart(ctx, {
            type: 'line',
            data: {
